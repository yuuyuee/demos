...
Status FileSystem::NewFileInputStream(const std::string filename, std::unique_ptr<InputStream>* result) {
...
Status FileSystem::NewFileOutputStream(const std::string filename, std::unique_ptr<OutputStream>* result) {
...
struct ZlibOptions {
    ...
    ZlibOptions(const long input_buffer_size, const long output_buffer_size,
                           const int flush_mode, const int  window_bits,
                           const int compression_level, const int  compression_method,
                           const int  mem_level, const int  compression_strategy)
...
#define CHECK_IN_RANGE(value, min, max) (((value) >= (min)) || ((value) <= (max)))
    ...
    static ZlibOptions Default() {
        return ZlibOptions(256 << 10, 256 << 10,
                           FlushMode::NO_FLUSH, kMaxWindowBits,
...
Status ZlibDeflate::ZlibDeflateImpl::operator()(const InputStream* in, OutputStream* out) {
    ...
    Status s;
    while ((s = in->Read(opts_.input_buffer_size, &input_buffer)).ok()) {
        ...
        do {
            ...
            ret = deflate(&zstream,utils::ZlibOptions::FINISH);
...
Status ZlibInflate::ZlibInflateImpl::operator ()(const InputStream* in, OutputStream* out) {
    ...
    while ((s = in->Read(opts_.input_buffer_size, &input_buffer)).ok()) {
        ...
        do {
            zstream.next_out = reinterpret_cast<unsigned char*>(const_cast<char*>(output_buffer.data()));
            zstream.avail_out = output_buffer.size();
            ret = inflate(&zstream, utils::ZlibOptions::FINISH);
...
bool Strtol(std::string str, long int* result, int base) {
    errno = 0;
    long int value = strtol(str.data(), NULL, base);
    if (errno)
        return false;
    *result = value;
    return true;
...
public:
    ...
    inline const uint64_t magic() const;
...
Status License::LicenseImpl::Generator(const std::string& identify, const uint64_t magic,
        const std::string& expired, std::string* cipher) {
    int year = 0, mon = 0, day = 0;
...
inline const uint64_t License::LicenseImpl::magic() const { return magic_; }
...
License::License(std::string cipher): impl_(new LicenseImpl(cipher)) {}

License::~License() {}

Status License::Generator(const std::string indentify, const uint64_t magic,
        const std::string expired, std::string* cipher) {
...
const uint64_t License::Magic() const { return impl_->magic(); }
