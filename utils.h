...
public:
    ...
    Status(error::Code code, const std::string& msg)
        : code_(code), msg_(msg) { CHECK_NE(code, error::OK); }
...
inline bool Status::ok() const { return code_ == error::OK; }

inline error::Code Status::code() const { return code_; }

inline const std::string& Status::message() const { return msg_; }

inline const Status& Status::operator=(const Status& o) {
    if (this != &o) {
        code_ = o.code_;
        msg_ = o.msg_;
    }
...
inline void Status::Clear() { *this = Status(); }
...
public:
    ...
    static Status CurrentDirectory(std::string* path);
...
inline bool FileSystem::IsAbsolutePath(const std::string& path) {
    return !path.empty() && path[0] == '/';
}

inline std::string FileSystem::Join(const std::string& a, const std::string& b) {
    if (a.empty()) return b;
    if (a[a.size() - 1] != '/') {
        return a + "/" + b;
    } else {
...
inline Status FileSystem::MoveFile(const std::string& src, const std::string& target) {
    RETURN_IF_ERROR(CopyFile(src, target));
    return DeleteFile(src);
}

Status Spawn(const std::string& cmd, int* child_ret = nullptr);
...
inline Status EncryptFile(const std::string& key, const std::string& fin, const std::string& fout) {
...
inline Status DecryptFile(const std::string& key, const std::string& fin, const std::string& fout) {
    std::unique_ptr<InputStream> in;
    RETURN_IF_ERROR(FileSystem::NewFileInputStream(fin, &in));
    std::unique_ptr<OutputStream> out;
    RETURN_IF_ERROR(FileSystem::NewFileOutputStream(fout, &out));
...
inline Status EncryptStringToFile(const std::string& key, const std::string& sin, const std::string& fout) {
    StringInputStream in(sin);
    std::unique_ptr<OutputStream> out;
    RETURN_IF_ERROR(FileSystem::NewFileOutputStream(fout, &out));
...
inline Status DecryptStringToFile(const std::string& key, const std::string& sin, const std::string& fout) {
    StringInputStream in(sin);
    std::unique_ptr<OutputStream> out;
    RETURN_IF_ERROR(FileSystem::NewFileOutputStream(fout, &out));
...
inline Status DeflateFile(const std::string& fin, const std::string& fout) {
    std::unique_ptr<InputStream> in;
    RETURN_IF_ERROR(FileSystem::NewFileInputStream(fin, &in));
    std::unique_ptr<OutputStream> out;
    RETURN_IF_ERROR(FileSystem::NewFileOutputStream(fout, &out));
...
inline Status InflateFile(const std::string& fin, const std::string& fout) {
    std::unique_ptr<InputStream> in;
    RETURN_IF_ERROR(FileSystem::NewFileInputStream(fin, &in));
    std::unique_ptr<OutputStream> out;
    RETURN_IF_ERROR(FileSystem::NewFileOutputStream(fout, &out));
...
inline Status DeflateStringToFile(const std::string& sin, const std::string& fout) {
    StringInputStream in(sin);
    std::unique_ptr<OutputStream> out;
    RETURN_IF_ERROR(FileSystem::NewFileOutputStream(fout, &out));
...
inline Status InflateStringToFile(const std::string& sin, const std::string& fout) {
    StringInputStream in(sin);
    std::unique_ptr<OutputStream> out;
    RETURN_IF_ERROR(FileSystem::NewFileOutputStream(fout, &out));
...
inline bool StrtoInt(const std::string& str, Tp* result, int base = 10) {
    static_assert(std::is_integral<Tp>::value, "template argument substitution failed");
    long long int value = 0;
    bool ret = internal::Strtoll(str, &value, base);
    if (!ret)
        return false;
...
class License {
public:
    License(const std::string& cipher);
    ~License();
