...
TEST(OAK_BUFFER_TEST, Initialize) {
  ...
  EXPECT_EQ(buffer3->capability, 100U);
  ...
  EXPECT_EQ(buffer4->capability, 100U);
...
TEST(OAK_BUFFER_TEST, ClearEmpty) {
  ...
  EXPECT_EQ(b1.capability, 100U);
  EXPECT_EQ(b1.size, 0U);
  EXPECT_TRUE(oak_buffer_empty(&b1));
  EXPECT_EQ(b2.capability, 100U);
...
TEST(OAK_BUFFER_TEST, Assign) {
  ...
  EXPECT_EQ(b1.capability, strlen(foo));
  EXPECT_EQ(b1.size, strlen(foo));

  EXPECT_EQ(b2.capability, strlen(foo));
  EXPECT_EQ(b2.size, strlen(foo));
...
TEST(OAK_BUFFER_TEST, Append) {
  ...
  EXPECT_EQ(b2.capability, (size_t) (strlen(foo) * 1.5));
  EXPECT_EQ(b2.size, strlen(foo));

  oak_buffer_append_str(&b2, bar, strlen(bar));
  EXPECT_EQ(b2.capability, (size_t) ((strlen(foo) + strlen(bar)) * 1.5));
...
TEST(OAK_BUFFER_TEST, Compare) {
  ...
  oak_buffer_append_str(&b2, bar, strlen(bar));
  EXPECT_EQ(oak_buffer_compare(&b1, &b2), 1);
...
TEST(OAK_PIECE_TEST, Compare) {
  ...
  EXPECT_EQ(oak_piece_compare(p1, p2), 1);
  EXPECT_EQ(oak_piece_compare(p3, p4), 1);
